...
export class EtaskTaskPanelComponent extends TaskPanelComponent {
  ...
  constructor(taskContentService: TaskContentService,
    ...
    // Render task forms with the application's own task content component, so that
    // fields resolve to our components (see EtaskFieldComponentResolverComponent).
    // `panelContentComponent` is an @Input on AbstractTaskPanelComponent and is read
    // in ngOnInit, so setting a default here is still picked up - and an explicit
    // binding from a parent template would continue to take precedence.
    this.panelContentComponent = EtaskTaskContentComponent;
  }
}
