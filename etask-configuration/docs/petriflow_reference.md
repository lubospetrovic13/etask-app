...
**C16 — A `button` action CANNOT read the value of a `text` or textarea field edited in the same interaction.**
Text and textarea fields send their value to the server on **blur** — which happens at the same instant as the button click. Both changes travel in one `setData` request, and the **button's event is processed before the text field's value is applied**. The action sees the previous (usually empty) value; clicking a second time works.

`phase="post"` does **not** fix this — "post" is relative to the button's *own* field, not to the whole batch.

`enumeration_map`, `multichoice_map` and `boolean` are **safe** — they send on change, so their value is already stored before the click. Only `text` and textarea are affected.

Two working fixes:
1. Put the logic in the **`set` event of the text field itself**, not in the button. The button then only re-renders, and the click still triggers it via blur.
2. Order the form so the **last input before the button is a select** — by the time it is clicked, the text fields have long lost focus.

```xml
<!-- ❌ button reads new_model_key / new_model_label: sees stale values -->
<!-- ✅ same form, but the provider select sits between the text fields and the button -->
<dataRef><id>new_model_key</id>    <layout><x>0</x><y>4</y>...</dataRef>
<dataRef><id>new_model_label</id>  <layout><x>2</x><y>4</y>...</dataRef>
<dataRef><id>new_model_provider</id><layout><x>4</x><y>4</y>...</dataRef>  <!-- select, commits instantly -->
<dataRef><id>btn_add</id>          <layout><x>0</x><y>5</y>...</dataRef>
```

Always guard the action so a stale read produces a clear message, never silent nothing:
```groovy
def key = ((new_model_key.value ?: "") as String).trim()
if (!key) { change message value { "Fill in the key and click again." }; return }
```

**C17 — Option map keys must not contain `.` or `$`.**
Options set at runtime (`change field options { [(key): label] }`) are stored as a MongoDB document, and Mongo forbids those characters in field names. Saving fails with:

```
Map key llama3.1:70b contains dots but no replacement was configured!
```

This hits most locally-hosted model tags (`llama3.1:70b`, `Qwen/Qwen2.5-32B`) and some external IDs (`gpt-4.1`) — a silent trap for **any dynamically populated codebook**.

Fix: put a **slug** in the options map, keep the real value in a source JSON field, and look it up by slug. Because the selected enum value is then the slug, the real key must live in a separate field:

```groovy
def slug = { k -> (k as String).replace(".", "__D__").replace('$', "__S__") }
change model options { list.collectEntries { m -> [(slug(m.key)): m.label] } }
// on select: resolve the real key back and store it separately
def hit = list.find { m -> slug(m.key) == model.value }
change model_key value { hit?.key }   // this is what leaves the process
```

Note the display text of an option also loses dots (`Llama 3.1 70B` renders as `Llama 31 70B`) — cosmetic, but misleading.

**C18 — Adding a data field to a net does NOT add it to existing cases.**
A case keeps the net version it was created with. After importing a net with a new field, `f.new_field` resolves to `null` in older cases and the action dies on:

```
Cannot set property 'value' on null object
```

That message **always** means `change <null> value` — a field in the import header that does not exist in this case. It never points at the real cause, so read it as "wrong case version", not "wrong code".

**The fix is a new case.** Guarding with `if (new_field != null) change new_field value { ... }` keeps the action alive but leaves the case half-populated — use it only for resilience, not as a substitute.

When reading fields from *another* case, prefer the null-safe form (see gotcha 33):
```groovy
def sv = { String id -> otherCase.dataSet[id]?.value }
```
...
| 33 | **`case.getFieldValue("id")` and `case.dataSet["id"]?.value` are NOT equivalent.** `getFieldValue` throws on a field that does not exist in that case's net version (see C18); `dataSet[id]?.value` returns `null`. When reading from another case — especially one that may predate a field — always use the null-safe form. |
| 34 | **Do NOT hardcode `workspace + "process_id"` — derive the prefix from the current case instead.** `workspace` exists only on multi-tenant eTask. On **single-tenant Netgrif Platform the variable does not exist** and `processIdentifier` carries no prefix (`ai_config`, not `ws/ai_config`), so `workspace + id` produces exactly the silent lookup failure it is supposed to prevent. This derivation works on both, and needs no special case for anonymous context (compare gotcha 39): <br>`def pid = useCase.processIdentifier`<br>`def prefix = pid.contains("/") ? pid.substring(0, pid.lastIndexOf("/") + 1) : ""`<br>Single-tenant yields `""`, multi-tenant `"ws/"`. Wrap it in a process function and pass the case in — `net_prefix(useCase)` — so every `findCases` / `findTasks` / `createCase` uses one implementation. Never a bare string literal either. |
...
| 39 | **`workspace` also fails for anonymous users** (anonymousRole=true) — it returns a different workspace path and `createCase`/`findCases` silently fails. This is a second, independent reason to use the derived prefix from gotcha 34 everywhere rather than only in anonymous processes. Prefer `lastIndexOf("/")` over the older subtraction trick (`pid - pid.split("/").last()`), which breaks when the process id appears twice in the path. |
...
| **IPC** | Each process is a fully independent `<document>`; cross-process interaction via action code only; process prefix **derived from `useCase.processIdentifier`**, never hardcoded `workspace + "id"` (C18, gotcha 34); cross-case reads via `dataSet[id]?.value`, not `getFieldValue` (gotcha 33); child receiver task on read arc (always alive); bulk finish: copy list → clear field → finish; `activePlaces` filter OK in QueryDSL, `dataSet` filter NOT OK; `assignTask` in button phase=pre is the one valid exception for child tasks |
...
### Renaming a task button: model yes, UI no (6.3.1)

`<event type="finish"><title>Podať</title><message>...</message></event>` parses,
imports and **is stored** — verified in Mongo:
...
`EventType` is exactly `ASSIGN` · `CANCEL` · `FINISH` · `DELEGATE`.

**But it never reaches the browser.** The task REST payload has no event data at all
(`assignPolicy, assignedUserPolicy, caseColor, caseId, caseTitle, dataFocusPolicy,
finishPolicy, icon, immediateData, layout, roles, stringId, title, transitionId,
users`), the net payload is only a reference, and `@netgrif/components` labels the
buttons from **global** i18n keys `tasks.view.finish` · `.cancel` · `.assign` ·
`.delegate`.

Consequence:

* per-task rename — **not reachable** without an engine/API change,
* app-wide rename — i18n override of those four keys,
* per-role removal — the permission table above, and it works today.
...
| 21 | **Derive the process prefix from `useCase.processIdentifier`, do NOT hardcode `workspace + "process_id"`** — `workspace` does not exist on single-tenant Netgrif Platform and silently breaks every lookup there. See gotcha 34 and C18. |
