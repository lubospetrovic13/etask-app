...
class Client:
    ...
    def call(self, method, path, body=None):
        ...
        headers = {"X-Auth-Token": self.token,
                   ...
                   # `defaultValue`, teda na to, co je v XML. Bez tejto hlavicky
                   # pouzije Spring locale JVM (tu `en`) a test porovnava
                   # slovenske ocakavania s anglickymi odpovedami - co je tiche,
                   # lebo prihlasenie aj dopyty funguju, len sa nic nenajde.
                   # Detail: starter, docs/PETRIFLOW_LEARNINGS.md B24.
                   "Accept-Language": "zz"}
        if data:
...
def wipe(cl):
    ...
    st, r = cl.post("/api/workflow/case/search?size=500",
                    {"process": [{"identifier": "hr/dovolenky/dv_ziadost"}]})
    cases = (r.get("_embedded") or {}).get("cases", [])
    for c in cases:
        cl.call("DELETE", f"/api/workflow/case/{c['stringId']}")
    print(f"dvcheck: zmazanych {len(cases)} casov hr/dovolenky/dv_ziadost")
...
def main():
    ...
    for name, cl, expected in [("zamestnanec", emp, True), ("veduci", boss, True),
        ...
        check(f"{name} {'vidi' if expected else 'nevidi'} kartu 'hr/dovolenky'",
              ("hr/dovolenky" in paths) == expected, paths)
    st, node = emp.get("/api/v2/uri/" + base64.b64encode(b"hr/dovolenky").decode())
    ...
    net = newest_net(emp, "hr/dovolenky/dv_ziadost")
    ...
    st, srch = Client("super@netgrif.com", os.environ.get("PF_PASS", "password")).post(
        "/api/workflow/case/search?size=5", {"process": [{"identifier": "it/service_desk/sd_menu"}]})
