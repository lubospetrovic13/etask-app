...
FAKTURA = "schvalovanie/faktury/fa_faktura"
OBJEDNAVKA = "schvalovanie/objednavky/ob_objednavka"
MENU = "schvalovanie/sc_menu"
NASTAVENIA = "nastavenia/sc_nastavenia"
...
HEADERS_FA = ("meta-title,schvalovanie/faktury/fa_faktura-fa_stav_label"
...
HEADERS_OB = ("meta-title,schvalovanie/objednavky/ob_objednavka-ob_stav_label"
...
def wipe(cl):
    ...
    # (`schvalovanie/fa_faktura` -> `schvalovanie/faktury/fa_faktura`), takze
...
def main():
    ...
    for name, cl, expected in [("zadavatel", zad, True), ("schvalovatel", sch, True),
        ...
        check(f"{name} {'vidi' if expected else 'nevidi'} kartu 'schvalovanie'",
              ("schvalovanie" in paths) == expected, paths)
    st, node = zad.get("/api/v2/uri/" + base64.b64encode(b"schvalovanie").decode())
    ...
    # identifikator tu cestu nesie - `schvalovanie/faktury/fa_faktura`.
    deti = {c["uriPath"]: c for c in (node.get("children") or [])}
    check("priecinok 'schvalovanie/faktury' existuje", "schvalovanie/faktury" in deti,
          sorted(deti))
    check("priecinok 'schvalovanie/objednavky' existuje",
          "schvalovanie/objednavky" in deti, sorted(deti))
    check("priecinok faktur ma vlastnu ikonu",
          (deti.get("schvalovanie/faktury") or {}).get("icon") == "receipt_long",
          (deti.get("schvalovanie/faktury") or {}).get("icon"))
    check("priecinok objednavok ma vlastnu ikonu",
          (deti.get("schvalovanie/objednavky") or {}).get("icon") == "shopping_cart",
          (deti.get("schvalovanie/objednavky") or {}).get("icon"))
    ...
    for cesta, kolko, kto in (("schvalovanie/faktury", len(VIEWS_FA), "faktur"),
                              ("schvalovanie/objednavky", len(VIEWS_OB), "objednavok")):
    ...
    for name, cl, expected in [("spravca (super)", riad, True),
        ...
        st, root = cl.get("/api/v2/uri/root")
        paths = [c["uriPath"] for c in root.get("children", [])]
        check(f"kartu 'nastavenia' {name} {'vidi' if expected else 'nevidi'}",
              ("nastavenia" in paths) == expected, paths)
